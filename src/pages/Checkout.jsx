          ...
          >
            <div className="bg-[#fffaf7] rounded-3xl p-8 shadow-sm border border-[#eadbd4]">
              ...
              <div className="grid md:grid-cols-2 gap-5">
                <input name="name" value={address.name} onChange={handleChange} placeholder="Full Name" required className="border border-[#eadbd4] rounded-xl p-4 outline-none focus:border-[#9A3F4D] bg-white" />
                <input name="phone" type="tel" value={address.phone} onChange={handleChange} placeholder="Phone Number" required pattern="[0-9]{10}" maxLength={10} className="border border-[#eadbd4] rounded-xl p-4 outline-none focus:border-[#9A3F4D] bg-white" />
                <input name="email" type="email" value={address.email} onChange={handleChange} placeholder="Email Address" className="border border-[#eadbd4] rounded-xl p-4 outline-none focus:border-[#9A3F4D] bg-white md:col-span-2" />
                <input name="house" value={address.house} onChange={handleChange} placeholder="House No / Street / Area" required className="border border-[#eadbd4] rounded-xl p-4 outline-none focus:border-[#9A3F4D] bg-white md:col-span-2" />
                <input name="city" value={address.city} onChange={handleChange} placeholder="City" required className="border border-[#eadbd4] rounded-xl p-4 outline-none focus:border-[#9A3F4D] bg-white" />
                <input name="state" value={address.state} onChange={handleChange} placeholder="State" required className="border border-[#eadbd4] rounded-xl p-4 outline-none focus:border-[#9A3F4D] bg-white" />
                <input name="pincode" value={address.pincode} onChange={handleChange} placeholder="Pincode" required pattern="[0-9]{6}" maxLength={6} className="border border-[#eadbd4] rounded-xl p-4 outline-none focus:border-[#9A3F4D] bg-white" />
              </div>
              ...
              {address.payment !== "COD" && (
                <div className="mt-5 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-2xl p-4 text-sm leading-6">
                  Online payment gateway abhi connect nahi hai. Filhaal testing ke liye order payment status Pending rahega.
                </div>
              )}
            </div>
            ...
            <aside className="bg-[#fffaf7] rounded-3xl p-6 shadow-sm border border-[#eadbd4] h-fit lg:sticky lg:top-28">
              ...
              <div className="space-y-4">
                {cartItems.length === 0 ? (
                  ...
                  cartItems.map((item) => (
                    <div
                      key={item.cartItemId || `${item._id || item.id}-${item.selectedSize || "Free Size"}`}
                      className="flex gap-4 border-b border-[#eadbd4] pb-4"
                    >
                      <img src={item.image} alt={item.name} className="w-20 h-24 object-cover object-top rounded-xl bg-[#FDEAE6]" />
                      <div className="flex-1">
                        <h3 className="font-bold text-[#5B3B32]">{item.name}</h3>
                        <p className="text-[#8b746b] text-sm mt-1">Size: {item.selectedSize || "Free Size"}</p>
                        <p className="text-[#8b746b] text-sm">Qty: {item.qty || 1}</p>
              ...
              <div className="mt-6 space-y-3 text-[#5B3B32]">
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <span>₹{Number(cartTotal || 0).toLocaleString("en-IN")}</span>
                </div>
              ...
              {appliedCoupon && (
                <div className="mt-5 bg-green-50 border border-green-200 rounded-2xl p-4">
                  ...
                  <p className="text-xs text-green-700 mt-1">
                    You are saving ₹{Number(discountAmount || 0).toLocaleString("en-IN")} on this order.
                  </p>
                </div>
              )}

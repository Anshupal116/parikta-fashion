      ...
      {open && (
        <div className="absolute right-0 top-14 z-[500] w-[360px] max-w-[calc(100vw-24px)] overflow-hidden rounded-3xl border border-[#eadbd4] bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-[#f1e6e1] px-5 py-4">
            ...
            {unreadCount > 0 && <button type="button" onClick={markAllRead} className="text-xs font-bold text-[#9A3F4D]">Mark all read</button>}
          </div>

          <div className="max-h-[420px] overflow-y-auto">
            {loading && !notifications.length ? <div className="px-6 py-12 text-center text-[#8b746b]">Loading...</div> :
             error && !notifications.length ? <div className="px-6 py-12 text-center text-red-600">{error}</div> :
             notifications.length ? notifications.slice(0, 6).map((item) => (
              <button key={item.id} type="button" onClick={() => openItem(item)} className={`flex w-full gap-3 border-b border-[#f6eeea] px-5 py-4 text-left hover:bg-[#fff9f6] ${item.read ? "bg-white" : "bg-[#fff5f1]"}`}>
          ...
          <button type="button" onClick={() => { setOpen(false); navigate("/admin-dashboard/notifications"); }} className="w-full bg-[#5B3B32] px-5 py-4 text-sm font-bold text-white">View All Notifications</button>
        </div>
      )}
    </div>
  );
